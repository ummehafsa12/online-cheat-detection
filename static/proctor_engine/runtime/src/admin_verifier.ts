...
import type { StudentTelemetryV2 } from "./student_engine.js";

type VerifierCallbacks = {
  getActiveStudentIds: () => string[];
  getVideoElement: (studentId: string) => HTMLVideoElement | null;
  getTelemetryHistory: (studentId: string) => StudentTelemetryV2[];
  onCheck?: (payload: {
    student_id: string;
    mismatch_score: number;
    sample_time_ms: number;
    matched_telemetry_time_ms: number;
    worker_id: number;
  }) => void;
  onFlag?: (payload: {
    student_id: string;
    ...
    details: string;
    samples: Array<{ ts: number; score: number }>;
  }) => void;
};
...
export class AdminTamperVerifier {
  private opts: Required<VerifierOptions>;
  private workerCores: ProctorCore[] = [];
  private callbacks: VerifierCallbacks | null = null;

  private workerTimers: number[] = [];
  private workerBusy: boolean[] = [];

  private initialized = false;
  private history = new Map<string, MismatchSample[]>();
  ...
  start(callbacks: VerifierCallbacks): void {
    if (!this.initialized) {
    ...
    const tickMs = Math.max(220, Math.round(1000 / Math.max(0.1, perWorkerChecks)));
  ...
  private async runWorkerCheck(workerId: number): Promise<void> {
    ...
    try {
      ...
      if (highMismatchCount >= 3 && avg >= this.opts.flagAvgThreshold) {
        this.cooldownUntil.set(studentId, now + this.opts.cooldownSec * 1000);
  ...
  private toTelemetryShape(studentId: string, analyzed: Awaited<ReturnType<ProctorCore["analyze"]>>): StudentTelemetryV2 {
    if (!analyzed) {
  ...
  private pickNearestTelemetry(history: StudentTelemetryV2[], targetTs: number, maxLagMs: number): StudentTelemetryV2 | null {
    if (!history || history.length === 0) {
      return null;
    }

    let best: StudentTelemetryV2 | null = null;
  ...
  private computeMismatchScore(adminData: StudentTelemetryV2, studentData: StudentTelemetryV2): number {
    ...
    const union = new Set([...adminLabels, ...studentLabels]);
    ...
    const weighted =
      ...
      personDelta * 0.10 +
      bannedMismatch * 0.15 +
      headPoseMismatch * 0.10 +
      gazeMismatch * 0.07 +
      lightingMismatch * 0.08;
